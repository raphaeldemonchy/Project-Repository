...
const SearchScreen = () => {
  ...
  if (data && data.employee) {
    ceoEmployees = data.employee.filter(employee => employee.fonction === 'CEO');
  }
        ...
        (data.employee.map((employeeInfo, index) => (
          <TouchableOpacity
            ...
            <View style={styles.employeeInfo}>
              <Text style={{fontWeight: 'bold'}}>{employeeInfo.name + ' ' + employeeInfo.surname}</Text>
              <Text>{employeeInfo.email}</Text>
              <Text>ID: {employeeInfo.id}</Text>
              <Text>{employeeInfo.fonction}</Text>
            </View>
